...
import { formatCoupon, formatDate, getStatusClass, daysUntil, formatCurrency } from '@/lib/utils';
...
export default function ProductDetailPage() {
      ...
      <div className="min-h-screen flex flex-col">
        ...
        <main className="flex-1 flex items-center justify-center">
          <div className="text-center">
            <p className="text-zinc-400 text-lg mb-4">{lang === 'zh' ? '产品未找到' : 'Product not found'}</p>
            <Link href="/" className="text-[#c8a97e] text-sm hover:underline">{t("detail.backToList", lang)}</Link>
          </div>
        </main>
        <Footer />
      </div>
  ...
  const similarProducts = mockProducts
    .filter(p => p.id !== product.id && (p.productType === product.productType || p.issuer === product.issuer))
    .slice(0, 3);
      ...
      <main className="flex-1">
        ...
        <section className="max-w-[1400px] mx-auto px-6 lg:px-10 pb-8">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <span className="px-2.5 py-1 rounded text-[11px] font-medium tracking-wider uppercase" style={{ backgroundColor: `${issuerInfo.color}20`, color: issuerInfo.color, border: `1px solid ${issuerInfo.color}30` }}>
              {typeInfo.abbr}
            </span>
            <span className="text-sm text-zinc-400">{exchangeInfo.flag} {exchangeInfo.name}</span>
              ...
              <div className="bg-[#111113] border border-zinc-800/60 rounded-xl p-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-6">
                  <div>
                    ...
                    <p className="text-2xl font-light text-[#c8a97e]">{product.couponRate ? `${(product.couponRate * 100).toFixed(2)}%` : '—'}</p>
                  ...
                  <div>
                    ...
                    <p className="text-sm text-zinc-300">{product.couponFrequency ? t(`freq.${product.couponFrequency}`, lang) : '—'}</p>
                  </div>
                  <div>
                  ...
                  <div className="relative h-16 bg-zinc-900 rounded-lg overflow-hidden">
                    ...
                    {product.knockInLevel && (
                      <div className="absolute top-0 bottom-0 bg-red-500/10 border-r-2 border-red-500/40" style={{ left: 0, width: `${product.knockInLevel}%` }}>
                        <span className="absolute top-1 right-2 text-[10px] text-red-400 font-medium">{t("product.knockIn", lang)} {product.knockInLevel}%</span>
                      </div>
                    )}
                    {/* Knock-out zone */}
                    {product.knockOutLevel && (
                      <div className="absolute top-0 bottom-0 bg-emerald-500/10 border-l-2 border-emerald-500/40" style={{ left: `${product.knockOutLevel}%`, right: 0 }}>
                        <span className="absolute top-1 left-2 text-[10px] text-emerald-400 font-medium">{t("product.knockOut", lang)} {product.knockOutLevel}%</span>
                      </div>
                    )}
                    {/* Strike line */}
                    {product.strikeLevel && (
                      <div className="absolute top-0 bottom-0 w-0.5 bg-[#c8a97e]" style={{ left: `${product.strikeLevel}%` }}>
                        <span className="absolute -top-5 left-1/2 -translate-x-1/2 text-[10px] text-[#c8a97e] font-medium whitespace-nowrap">{t("product.strike", lang)}</span>
                      </div>
                    )}
                    ...
                    <tbody>
                      {product.underlyings.map(u => {
                        const change = u.initialLevel && u.currentLevel
                          ...
                          : null;
                        return (
                          <tr key={u.ticker} className="border-b border-zinc-800/50">
              ...
              <div className="bg-[#111113] border border-zinc-800/60 rounded-xl p-6">
                ...
                <div className="space-y-2">
                  {product.termSheetUrl && (
                    <a href={product.termSheetUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2.5 bg-zinc-800/30 rounded-lg hover:bg-zinc-800/60 transition-colors group">
                  ...
                  {product.prospectusUrl && (
                    <a href={product.prospectusUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2.5 bg-zinc-800/30 rounded-lg hover:bg-zinc-800/60 transition-colors group">
                      <span className="text-lg">📋</span>
                      <span className="text-xs text-zinc-400 group-hover:text-zinc-200">{t("product.prospectus", lang)}</span>
                      <span className="text-zinc-600 ml-auto">↗</span>
                    </a>
                  )}
                  <a href={product.sourceUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 px-3 py-2.5 bg-zinc-800/30 rounded-lg hover:bg-zinc-800/60 transition-colors group">
                    <span className="text-lg">🔗</span>
          ...
          <section className="max-w-[1400px] mx-auto px-6 lg:px-10 pb-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {similarProducts.map(p => (
                <div key={p.id}>
                  {/* Inline simplified product card to avoid import issues */}
                  <Link href={`/product/${p.id}`} className="block bg-[#111113] border border-zinc-800/60 rounded-xl p-5 hover:border-zinc-700/80 transition-all">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="px-2 py-0.5 rounded text-[10px] font-medium" style={{ backgroundColor: `${ISSUER_INFO[p.issuer].color}20`, color: ISSUER_INFO[p.issuer].color }}>
                        {PRODUCT_TYPE_INFO[p.productType].abbr}
                      </span>
                      <span className="text-xs text-zinc-500">{EXCHANGE_INFO[p.exchange].flag} {lang === 'zh' ? ISSUER_INFO[p.issuer].nameZh : ISSUER_INFO[p.issuer].name}</span>
                    </div>
                    <p className="text-xs text-zinc-300 mb-2 line-clamp-2">{lang === 'zh' && p.productNameZh ? p.productNameZh : p.productName}</p>
